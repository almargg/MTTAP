...
#TODO: Try learnable positional encoding
def posenc(x, min_deg, max_deg):
    """
    Cat x with a positional encoding of x with scales 2^[min_degr, max_deg-1].
    Compute cos(x) as sin(x + pi/2) to do one vectorised call to sin([x, x+pi/2]).
    """
    if min_deg == max_deg:
        return x
    scales = torch.tensor(
    ...
    four_feat = torch.sin(torch.cat([xb, xb + torch.pi / 2], dim=-1))
